...
public class ExcelTools : EditorWindow
{
	...
	private static void Convert()
	{
		foreach(string assetsPath in excelList)
		{
			...
			string output="";
			if(indexOfFormat==0){
                if (excelPath.Contains(local))
                {
                    output = excelPath.Replace(".xlsx", ".json");
                    output = output.Replace("ExcelFile/JsonLocal", "Download/Config/Json");
                }
                else if (excelPath.Contains(server))
                {
                    output = excelPath.Replace(".xlsx", ".json");
                    output = output.Replace("ExcelFile/JsonServer", "DownloadServer/Config/Json");
                }
                else { output = excelPath.Replace(".xlsx", ".json"); }
				excel.ConvertToJson(output,encoding);
			}else if(indexOfFormat==1){
                if (excelPath.Contains(local))
                {
                    output = excelPath.Replace(".xlsx", ".csv");
                    output = output.Replace("ExcelFile/CsvLocal", "Download/Config/Csv");
                }
                else if (excelPath.Contains(server))
                {
                    output = excelPath.Replace(".xlsx", ".csv");
                    output = output.Replace("ExcelFile/CsvServer", "DownloadServer/Config/Csv");
                }
                else
                {
                    output = excelPath.Replace(".xlsx", ".csv");
                ...
                if (excelPath.Contains(local))
                {
                    ...
                    output = output.Replace("ExcelFile/XmlLocal", "Download/Config/Xml");
                }
                else if (excelPath.Contains(server))
                {
                    output = excelPath.Replace(".xlsx", ".xml");
                    output = output.Replace("ExcelFile/XmlServer", "DownloadServer/Config/Xml");
                }
                else
                {
                    output = excelPath.Replace(".xlsx", ".xml");
			...
			}else if (indexOfFormat == 3)
            {
                if (excelPath.Contains(local))
                {
                    ...
                    output = output.Replace("ExcelFile/LuaLocal", "Download/Config/Lua");
                }
                else if (excelPath.Contains(server))
                {
                    output = excelPath.Replace(".xlsx", ".lua");
                    output = output.Replace("ExcelFile/LuaServer", "DownloadServer/Config/Lua");
                }
                else
                {
                    output = excelPath.Replace(".xlsx", ".lua");
