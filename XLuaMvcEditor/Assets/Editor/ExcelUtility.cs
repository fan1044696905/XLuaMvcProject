...
public class ExcelUtility
{
	...
	public void ConvertToJson (string JsonPath, Encoding encoding)
	{
		...
		for (int i = 1; i < rowCount; i++) {
			...
			for (int j = 0; j < colCount; j++) {
                ...
                if (mSheet.Rows[i][j].GetType() == typeof(double))
                {
                    double num = (double)(mSheet.Rows[i][j]);
                    if ((int)num == num)
                        mSheet.Rows[i][j] = (int)num;
                }
    ...
    private void CheckFloader(string path)
    {
        int index = path.LastIndexOf('/');
        string tempPath = path.Remove(index, path.Length - index);
